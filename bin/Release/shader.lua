...
local polar = C.newShader([[#ifdef GL_ES
...
local polar = C.newShader([[
uniform sampler2D tex;
void main(void)
{
	vec4 col = texture2D(tex, gl_TexCoord[0].xy);
	//float full = col.r + col.
	//gl_FragColor = vec4(fract(full/32512.5), full/255.0/255.0, col.a, col.b);
	//gl_FragColor = vec4(col.r*0.5+col.b*0.5, col.g, col.a, 1.0);
	gl_FragColor = vec4(fract(col.r*2.0), col.g, col.r*2.0-1.0, col.b);
	gl_FragColor = vec4(col.r*2.0-1.0, col.g, (col.r*2.0), col.b);
}
]])
noisetex = C.newImage('cool1.png')
E:new(screen):draw(function()
	polar:bind()
	polar:set('resolution', 512, 512)
	polar:set('time', time)
	polar:set('noise', 0)
	noisetex:drawxy(0,0,512,512)
	polar:unbind()
end)
