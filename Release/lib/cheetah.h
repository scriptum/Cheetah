...
int getMouseX(void);
int getMouseY(void);
const unsigned char *getKeyState(void);
...
void fontEnableSDF(Font *f);
void fontDisableSDF(Font *f);
...
void __attribute__((optimize("-O3"))) fontPrintf(Font *currentFont, const char *str, float x, float y, float maxw, int align);
...
Font *newFont(const char *filename, bool scalable);
